...
class Trigger:
    ...
    Trigger configuration:
        ...
        that allow defining the exact conditions when a triggering should occur:
        ...
        for:
            ...
            Hours:Minutes:Seconds. The trigger delay is applied individually for 
            each specified trigger threshold level:
        min_interval
    ...
    def _log_and_check_trigger(self, value):
        ...
        # Ignore the level exceedance if required time span (parameter 'for') is
        # not met.
        ...
        for index, threashold in enumerate(self.above):
            ...
            if self.first_trigger_time_above[index] is not None and \
                   current_time>=self.first_trigger_time_above[index]+self.for_:
        ...
        for index, threashold in enumerate(self.below):
            ...
            if self.first_trigger_time_below[index] is not None and \
                   current_time>=self.first_trigger_time_below[index]+self.for_:
