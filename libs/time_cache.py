...
if __name__ == "__main__":
    ...
    read_data_many_times(cache, cache_size=3, nbr_iterations=1)
    fill_cache(cache, cache_size=3)
    read_data_many_times(cache, cache_size=3, nbr_iterations=3)

    logger.info("**** Cache with max_read=2 ****")
    cache = TimeCache(max_read=2)
    fill_cache(cache, 3)
    read_data_many_times(cache, cache_size=3, nbr_iterations=3)

    logger.info("**** Cache with ttl=0.5 ****")
    cache = TimeCache(ttl=0.5)
    fill_cache(cache, 3)
    read_data_many_times(cache, cache_size=3, nbr_iterations=3)


    logger.info("**** Cache with max_read=2 and ttl=1.0, random access ****")
    cache = TimeCache(ttl=0.5, max_read=1)
    fill_cache(cache, 3)
