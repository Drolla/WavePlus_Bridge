...
class _LogDbCsv:
    ...
    def insert(self, data_set):
        ...
        if self.DEBUG:
            print("_LogDbCsv - insert:", data_set)
        ...
        for index, label in enumerate(self.labels):
            value = self._NAN if label not in data_set or data_set[label]=="" \
                             else data_set[label]
            ...
            # Delta log: Store value in function of the previous value
                ...
                # Delta log, previous value is available
                    ...
                    if value == value:
                        csv_data.append("" if value==last_value else str(value))
                    
                    # Delta log, new value is NaN
                    else:
                        csv_data.append("n" if last_value==last_value else "")
...
class LogDB:
    ...
    def __init__(self, label_specs, file=None,
                 ...
                 log_delta = False):
        if self.DEBUG:
            print("LogDB, Init (file=", file, ", label=", label_specs, ")")
    ...
    def get_labels_from_spec(self, label_specs):
        ...
        if self.DEBUG:
            print("LogDB - get_labels_from_spec:", label_specs)
        csv_file_data = []
    ...
    def get_csv(self, label_specs=".*",
        ...
        if self.DEBUG:
            print("LogDB - get_csv:", label_specs)
        csv_file_data = []
            ...
            if (pos+1) % decimation_factor == 0:
                ...
                for data_index, data_array in enumerate(data_matrix[1:]):
                    if sum_values[data_index] == sum_values[data_index] and \
                        ...
                        data_avg = sum_values[data_index]/sum_length[data_index]
